...
import { CampoDeCor, GradeDeOpcoes } from "~/components/user-settings/perfil/campos";
 ...
 */
export const EnfeitesAba: React.FC<EnfeitesAbaProps> = ({ rascunho, definir }) => (
  <div className="space-y-6">
    <GradeDeOpcoes
      ...
      amostra={(id) => (
        <span className="text-base" style={{ fontFamily: familiaDaFonte(id) ?? undefined }}>
          Ana
        </span>
      )}
    />
    ...
    <GradeDeOpcoes
      ...
      amostra={(id) => {
        ...
        return (
          <span className={cn("text-base font-bold", enfeite.className)} style={enfeite.style}>
            Ana
          </span>
        );
      }}
    ...
    <GradeDeOpcoes
      ...
      amostra={(id) => {
        ...
        return <span className={cn("text-xs", placa && "gc-placa", placa)}>Ana</span>;
      }}
    />
  </div>
);
 ...
 * foto redonda.
 */
const Amostra: React.FC<{ familia: string; id: string }> = ({ familia, id }) => {
  ...
  return (
    <span className="relative block size-7 rounded-full bg-surface-4">
      {classe && (
        <span
          aria-hidden
          className={cn("gc-camada", familia === "moldura" && "gc-camada--moldura", classe)}
