...
export const SecaoDeServidores: React.FC<SecaoDeServidoresProps> = ({ botId, link }) => {
  ...
  return (
    <>
      {servidores.isLoading && <p className="text-sm text-ink-faint">Carregando…</p>}
          ...
          {servidores.data.map((servidor) => (
            ...
            >
              {servidor.iconUrl ? (
                ...
                <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-surface-4 text-10 font-bold uppercase">
                  {servidor.name.slice(0, 2)}
                </span>
              )}

              <span className="min-w-0 flex-1 truncate text-sm">{servidor.name}</span>
