  ...
  return (
    <div className="max-w-2xl pb-10">
      ...
      {porCategoria.map((categoria) => (
        <Secao key={categoria.id} id={`avancado-${categoria.id}`} titulo={categoria.nome}>
      ...
      {!porCategoria.length && (
        <p className="mt-6 text-sm text-ink-faint">Nenhuma configuração com esse nome.</p>
      )}
    </div>
  );
};
...
const LinhaDeAjuste: React.FC<{ ajuste: Ajuste; onIr: () => void }> = ({ ajuste, onIr }) => (
  <div className="flex items-center gap-3 border-b border-divisor px-3 py-2.5 last:border-b-0">
    <div className="min-w-0 flex-1">
      <p className="text-sm font-medium">{ajuste.rotulo}</p>
      <p className="mt-0.5 text-xs text-ink-faint">{ajuste.detalhe}</p>
    </div>
