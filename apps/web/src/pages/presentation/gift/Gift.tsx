...
export const Gift: React.FC = () => {
  ...
  const body = () => {
    ...
    if (!gift)
      return (
        <>
          <h1 data-gc="gift.gift.h1" className="text-xl font-bold">{t("configuracoes.subscription.giftNotFoundTitle")}</h1>
          <p data-gc="gift.gift.p" className="mt-2 text-sm text-ink-muted">{t("configuracoes.subscription.giftNotFoundDetail")}</p>
        </>
      );
    ...
    if (gift.claimed)
      return (
        <>
          <h1 data-gc="gift.gift.h1--2" className="text-xl font-bold">{t("configuracoes.subscription.giftUsedTitle")}</h1>
          <p data-gc="gift.gift.p--2" className="mt-2 text-sm text-ink-muted">{t("configuracoes.subscription.giftUsedDetail")}</p>
        </>
      );
    ...
    if (hasPlan)
      return (
        <>
          <h1 data-gc="gift.gift.h1--3" className="text-xl font-bold">{t("configuracoes.subscription.giftHasPlanTitle")}</h1>
          <p data-gc="gift.gift.p--3" className="mt-2 text-sm text-ink-muted">
            {until
              ? t("configuracoes.subscription.giftHasPlanDetail", { plan: PLAN_NAME, date: day(until) })
              : t("configuracoes.subscription.giftUsedDetail")}
          </p>

          <Button data-gc="gift.gift.button" className="mt-6 w-full" onClick={() => void copyLink()}>
            {t("configuracoes.subscription.giftCopyLink")}
          </Button>
          <Button data-gc="gift.gift.button--2" className="mt-2 w-full" variant="ghost" onClick={() => navigate("/channels")}>
    ...
    return (
      <>
        <h1 data-gc="gift.gift.h1--4" className="text-xl font-bold">{t("configuracoes.subscription.giftLinkTitle", { plan: PLAN_NAME })}</h1>

        {user && mine.data ? (
          <GiftReview data-gc="gift.gift.gift-review" code={clean} gift={mine.data} onClose={() => navigate("/channels")} />
        ) : (
          <>
            ...
            <p data-gc="gift.gift.p--6" className="mt-3 font-mono text-xs tracking-wider text-ink-faint">{prettyGiftCode(clean)}</p>

            <Button data-gc="gift.gift.button--3" className="mt-6 w-full" onClick={() => navigate("/login", { state: { from: `/gift/${clean}` } })}>
