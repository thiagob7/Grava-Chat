...
const rosa = (x, y, r, delay = 0) => `<g transform="translate(${x} ${y})"><g>
  <animateTransform attributeName="transform" type="scale"
    values="1;1.07;1" dur="6s" begin="${delay}s" repeatCount="indefinite"
    calcMode="spline" keyTimes="0;0.5;1" keySplines=".4 0 .6 1;.4 0 .6 1"/>
  <circle r="${r}" fill="${CREAM}" stroke="${SHADOW}" stroke-width="${r * 0.13}"/>
  <path d="M${-r * 0.6} ${r * 0.22} A${r * 0.6} ${r * 0.6} 0 1 1 ${r * 0.48} ${-r * 0.3}"
        fill="none" stroke="${SHADOW}" stroke-width="${r * 0.16}" stroke-linecap="round"/>
  <path d="M${-r * 0.3} ${r * 0.26} A${r * 0.34} ${r * 0.34} 0 1 1 ${r * 0.28} ${-r * 0.06}"
        fill="none" stroke="${SHADOW}" stroke-width="${r * 0.16}" stroke-linecap="round"/>
</g></g>`;

const leaf = (x, y, g, e = 1) =>
  `<ellipse rx="${6.5 * e}" ry="${2.7 * e}" fill="${CREAM}" stroke="${SHADOW}"
     stroke-width="1" transform="translate(${x} ${y}) rotate(${g})"/>`;

const corner = `<g id="canto">
  <path d="M48 12 C32 12 20 20 14 30 C10 37 10 42 10 48"
        fill="none" stroke="${BRANCH}" stroke-width="3.2" stroke-linecap="round"/>
  <path d="M48 22 C36 22 27 28 23 36 C20 41 20 44 20 48"
        fill="none" stroke="${BRANCH}" stroke-width="1.4" stroke-linecap="round" opacity=".6"/>
  ${leaf(40, 9, -22)}${leaf(9, 40, -68)}
  ${rosa(19, 19, 14)}
</g>`;

const beira = `<g id="beira">
  <path d="M48 15 C82 2 116 30 150 15 C184 0 218 28 252 15"
        fill="none" stroke="${BRANCH}" stroke-width="3" stroke-linecap="round"/>
  <path d="M48 22 C84 10 116 37 150 22 C184 7 216 34 252 22"
        fill="none" stroke="${BRANCH}" stroke-width="1.3" stroke-linecap="round" opacity=".55"/>
  ${leaf(100, 10, 28)}${leaf(150, 26, 0)}${leaf(200, 10, -28)}
</g>`;

writeFileSync(
  process.argv[2],
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${L} ${L}" width="${L}" height="${L}" role="presentation">
  <defs>${corner}${beira}</defs>
  <use href="#canto"/>
